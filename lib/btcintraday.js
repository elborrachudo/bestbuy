...
import { sbUpsert, sbDelete, sbSelect } from './tokens.js';

// Single source of truth for the rolling minute window (Fase 7 / Bloco 5). Change to 30/90 here
// and the import `since`, the prune, and the frontend CS guard all follow. ~60d of 1m ≈ 86k rows.
export const INTRADAY_WINDOW_DAYS = 60;
...
// btc_1m to the last INTRADAY_WINDOW_DAYS days. Best-effort — the caller wraps this in try/catch.
export async function rollIntraday(base, key) {
  const now = Math.floor(Date.now() / 1000);
  const summary = {};
  // 1m: re-fetch the last ~18h (covers the ≤8h gap between cron runs), prune past the window.
  const m = await fetchBitstampOhlc(60, now - 18 * 3600, 2);
  const mAll = await upsertCandles(base, key, 'btc_1m', m);
  await sbDelete(base, key, 'btc_1m', `ts=lt.${encodeURIComponent(iso(now - INTRADAY_WINDOW_DAYS * 86400))}`);
...
// Close the live DAILY candle of btc_history with REAL OHLC (Fase 7 / Bloco 5 — fixes "vela de
// hoje flat"). OHLC is aggregated from THE SAME DAY's hourly candles (already stored in btc_1h):
// open = first 1h open, high = max, low = min, volume = sum; the live price is merged as close
// (and into high/low). Only writes the supplied `day` (the live day) — past days keep their
// Bitstamp OHLC untouched. Falls back to close-only when no hourly data exists for the day.
export async function closeDailyCandle(base, key, day, live) {
  const start = `${day}T00:00:00Z`, end = `${day}T23:59:59.999Z`;
  let rows = [];
  try {
    rows = await sbSelect(base, key,
      `btc_1h?ts=gte.${encodeURIComponent(start)}&ts=lte.${encodeURIComponent(end)}&select=ts,open,high,low,close,volume&order=ts.asc`);
  } catch (e) { /* fall through to close-only */ }
  const row = { date: day, close: live, source: 'coingecko-live' };
  if (rows && rows.length) {
    let high = -Infinity, low = Infinity, vol = 0;
    for (const r of rows) { const h = Number(r.high), l = Number(r.low), v = Number(r.volume) || 0; if (h > high) high = h; if (l < low) low = l; vol += v; }
    if (live != null && isFinite(live)) { if (live > high) high = live; if (live < low) low = live; }
    const open = Number(rows[0].open);
    if (isFinite(open)) row.open = open;
    if (isFinite(high)) row.high = high;
    if (isFinite(low)) row.low = low;
    row.volume = vol; row.source = 'cron-1h-agg';
  }
  await sbUpsert(base, key, 'btc_history', [row], 'date');
  return row;
}
