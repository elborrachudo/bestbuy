...
// …but a single-component (emissions-only, market-only) pillar is shrunk toward 5.0:
//   raw 9 → 5 + (9-5)*0.5 = 7.0, so emissions alone can't push to STRONG BUY.
ok('fundamentals coverage halves a lone emissions component', near(scoreFundamentals(null, 9, 'none'), 7.0));
ok('fundamentals coverage pulls toward neutral, never to the raw', scoreFundamentals(null, 9, 'none') < 9);
