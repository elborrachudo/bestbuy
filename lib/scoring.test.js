...
// Coverage factor: a 2-component pillar at full coverage is unchanged…
ok('fundamentals = 60·tvlRev + 40·emissions (full coverage)', near(scoreFundamentals(5, 9, 'none'), 6.6));
// …but an emissions-only (market-only) pillar earns NO credit above neutral: low
// inflation alone is the absence of a weakness, not a fundamental strength → capped 5.0.
ok('emissions-only fundamentals capped at neutral (no credit above 5)', scoreFundamentals(null, 9, 'none') === 5.0);
ok('emissions-only never reaches the raw emissions value', scoreFundamentals(null, 9, 'none') < 9);
// High inflation alone still penalizes (below-neutral passes through, not capped).
ok('emissions-only high inflation still penalizes below neutral', near(scoreFundamentals(null, 2, 'none'), 3.5));
ok('fundamentals null only when tvlRev & emissions both null', scoreFundamentals(null, null, 'none') === null);
...
console.log('\nbuildReading');
{
  ...
  ok('B: reweighted true (no DeFi data)', b.reweighted === true);
  ...
  // Case D: native L1 token — no protocol slug, but chain-level TVL/fees present
  // (hasDefiChain) → real Fundamentals, not market-only and not capped at neutral.
  const d = buildReading({
    ...
    circSupply: 120e6, hasDefiSlug: false, hasDefiChain: true,
  });
