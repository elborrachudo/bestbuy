...
import {
  scorePriceVsMas, scoreDistFromLow, scoreRsi, scoreTvlRevenue, scoreEmissions,
...
ok('distFromLow at 90d low ≈ 10', near(scoreDistFromLow(50, 50, 150), 10));
ok('distFromLow at 90d high ≈ 0', near(scoreDistFromLow(150, 50, 150), 0));
ok('distFromLow midpoint ≈ 5', near(scoreDistFromLow(100, 50, 150), 5));
...
// ── Combine + verdict ─────────────────────────────────────────────────────────
console.log('\ncombine + verdict');
ok('full weights all-10 = 10', computeFinalScore({ priceMa:10, distLow:10, rsi:10, tvlRev:10, emissions:10 }, false) === 10);
ok('reweighted ignores tvl/emissions',
   computeFinalScore({ priceMa:10, distLow:10, rsi:10, tvlRev:0, emissions:0 }, true) === 10);
...
console.log('\nbuildReading');
{
  // Case A: token above both MAs but sitting at its 90d low, oversold RSI,
  // growing TVL, cheap, low emissions → should score high (strong buy region).
  const a = buildReading({
    price: 120, ma50: 100, ma200: 90,
    high90: 200, low90: 118, rsi14: 25,
  ...
  const c = buildReading({
    price: 1, ma50: 1, ma200: 1, high90: 2, low90: 0.5, rsi14: 50,
    tvlNow: null, tvl30dAgo: null, holdersRevenue: null,
