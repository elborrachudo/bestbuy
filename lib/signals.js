...
export function buildSignal(cur, prev, side) {
...
// Generate the full signal list for one token's chronological readings. The RSI
// trigger + 30-day per-side cooldown decide the COUNT; confluence only grades.
// `readings` must be ascending by fetched_at and carry: fetched_at, rsi_14,
// stochrsi_14, macd_histogram, score_below_high, score_fundamentals, score_activity,
// price. Returns signal objects (without token_id / is_backfill — caller stamps).
export function generateSignals(readings) {
  const out = [];
  ...
  for (let i = 0; i < readings.length; i++) {
    ...
    const last = side === 'BUY' ? lastBuy : lastSell;
    if (last != null && (t - last) / DAY_MS < COOLDOWN_DAYS) continue;   // cooldown

    out.push(buildSignal(cur, i > 0 ? readings[i - 1] : null, side));
    if (side === 'BUY') lastBuy = t; else lastSell = t;
  }
  return out;
}

// Live single-step: does the newest reading fire, given the previous reading (for
// crossovers) and the most recent BUY/SELL signal dates (for cooldown)? Returns a
// signal object or null.
export function detectLiveSignal(cur, prev, lastBuyDate, lastSellDate, nowIso) {
  ...
  const last = side === 'BUY' ? lastBuyDate : lastSellDate;
  if (last != null && (t - new Date(last).getTime()) / DAY_MS < COOLDOWN_DAYS) return null;

  return buildSignal({ ...cur, fetched_at: nowIso }, prev, side);
}
