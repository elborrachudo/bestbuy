...
// the score is shrunk toward the neutral 5.0 — so a market-only token (e.g. XRP)
// whose Fundamentals is emissions-only can no longer alone push to STRONG BUY.
//   adjusted = 5 + (raw - 5) * coverage,  coverage = nComponentsPresent / 2
// null only when neither TVL/revenue nor emissions exists (pillar-level reweight).
