...
// ── Pillars ───────────────────────────────────────────────────────────────────
// Three weighted pillars on top of the five sub-scores.
export const W_FUNDAMENTALS = { tvlRev: 60, emissions: 40 };
export const W_TECHNICALS   = { priceMa: 45, belowHigh: 35, rsi: 20 };
export const W_PILLARS      = { fundamentals: 45, technicals: 35, sentiment: 20 };
...
// Market-only tokens (no TVL/revenue) fall back to emissions alone, so a token like
// XRP still gets a Fundamentals value instead of riding on Technicals only. null
// only when neither TVL/revenue nor emissions exists.
export function scoreFundamentals(tvlRev, emissions, supplyMechanism) {
  const em = applySupplyModifier(emissions, supplyMechanism);
  return weightedBlend({ tvlRev, emissions: em }, W_FUNDAMENTALS);
}
...
// Blend the three pillars (F 45 / T 35 / S 20); null pillars drop & renormalize.
export function blendPillars(fundamentals, technicals, sentiment) {
  return round1(weightedBlend({ fundamentals, technicals, sentiment }, W_PILLARS));
}
...
export function buildReading(inputs) {
  ...
  // Three pillars, then blend (sentiment stubbed null until a feed is added).
  ...
  const final = blendPillars(fundamentals, technicals, sentiment);
