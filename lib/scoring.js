...
export function buildReading(inputs) {
  ...
  let tvlRev = null;
  // Fundamentals data comes from a DeFi protocol slug OR, for native L1 tokens,
  // from chain-level TVL/fees (ETH/SOL/AVAX). Either counts as a real source.
  const fundamentalsAvailable = (!!inputs.hasDefiSlug || !!inputs.hasDefiChain) &&
