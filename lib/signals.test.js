...
import { classifyPhase, phaseAllowsBuy, phaseAllowsSell } from './cycle.js';
...
console.log('\ncycle phase');
{
  // steady uptrend above the long MA → rise
  const up = Array.from({length: 60}, (_, i) => 100 + i * 2);
  ok('uptrend above MA → rise', classifyPhase(up, 59).phase === 'rise');
  // parabolic blow-off far above MA with strong momentum → euphoria
  const eup = Array.from({length: 60}, (_, i) => 100 * Math.pow(1.05, i));
  ok('parabolic far above MA → euphoria', classifyPhase(eup, 59).phase === 'euphoria');
  // sustained downtrend below MA, still falling → correction
  const down = Array.from({length: 60}, (_, i) => 200 - i * 2);
  ok('downtrend below MA → correction', classifyPhase(down, 59).phase === 'correction');
  // fell then flattened below MA for >30d → accumulation (below MA, momentum ≥ 0)
  const acc = Array.from({length: 60}, (_, i) => (i < 25 ? 200 - i * 5 : 75));
  ok('flat-below-MA → accumulation', classifyPhase(acc, 59).phase === 'accumulation');
}
