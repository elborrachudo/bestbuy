...
import { classifyLatest, classifySeries, phaseAllowsBuy, phaseAllowsSell } from './cycle.js';
...
console.log('\ncycle phase (4-indicator + hysteresis)');
{
  ...
  // deep drawdown STILL FALLING (mom30<0), no prior hot top → correction (don't buy knife)
  const knife = Array.from({length: 120}, (_, i) => 200 - i * 1.2);
  ok('deep drawdown still falling → correction', classifyLatest(knife).phase === 'correction');
