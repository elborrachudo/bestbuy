...
async function cgJson(url, apiKey, { retries = 3, delayMs = 2000 } = {}) {
  for (let attempt = 0; ; attempt++) {
    try {
      const res = await fetch(url, { headers: cgHeaders(apiKey) });
      if (res.status === 429 && attempt < retries) { await sleep(delayMs * (attempt + 1)); continue; }
...
//
// opts: { topN=100, pauseMs=1400, onProgress }
...
export async function buildDominanceSeries(apiKey, opts = {}) {
  ...
  const pauseMs = opts.pauseMs == null ? 1400 : opts.pauseMs;

  const top = await fetchTopCoins(apiKey, topN);
  const topIds = top.map((c) => c.id);
  // Coin set = top-N ∪ {bitcoin, ethereum} ∪ stablecoins (deduped, order preserved).
  const ids = [...new Set([...topIds, 'bitcoin', 'ethereum', ...STABLE_IDS])];
  ...
  if (!btc || !eth) throw new Error(`missing btc/eth series (btc=${!!btc} eth=${!!eth})`);
  ...
  // Calibration: present authoritative total ÷ present basket sum at the latest common day.
  ...
  const calib = (globalNow.total && basketLatest > 0) ? globalNow.total / basketLatest : 1;
