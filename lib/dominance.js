...
//   • Per-coin daily market cap comes from /coins/{id}/market_chart?days=365 (NO interval).
//     The demo tier 401s on days=max and on interval=daily (both paid-only), so 365 — the free
//     maximum — is the hard depth ceiling. We store whatever comes back (newer coins yield
//     fewer points); the caller reports the real first→last.
...
// Daily market-cap series for one coin → Map<dateStr, mcap>. The free/demo tier rejects
// days=max and the interval=daily param (both 401 — paid-only), so we request days=365 (the
// free maximum) and OMIT interval: CoinGecko auto-returns daily granularity for ranges ≥90d.
// Keeps whatever is returned (newer coins yield <365 points). Last sample of each day wins.
export async function fetchCoinDailyMcap(id, apiKey) {
  const url = `${CG_BASE}/coins/${id}/market_chart?vs_currency=usd&days=365`;
