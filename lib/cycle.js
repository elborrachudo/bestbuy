...
// US M2 money supply from FRED's KEYLESS public CSV (no API key, no paid source) as the
// global-liquidity proxy — the dominant crypto-liquidity driver. Monthly, slow-moving.
// Used ONLY as a displayed CONFIRMER (expanding M2 = tailwind for dip-buys; contracting =
// headwind); it does NOT change the phase gate. Server-side only (needs egress).
export async function getM2Monthly() {
  // FRED 403s header-less/automated requests, so send a browser-like UA + Accept.
  const res = await fetch('https://fred.stlouisfed.org/graph/fredgraph.csv?id=M2SL&cosd=2022-01-01', {
    headers: {
