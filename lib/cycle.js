...
function rawPhase(d, recentMaxMayer) {
  ...
  // Capitulation = ACCUMULATION: an extreme drawdown (≳60% off ATH, and cheap by Mayer) is
  // BTC's historically-cheap bottom zone, so it's accumulation even while still falling — this
  // OVERRIDES the falling-knife correction below (which is for the shallower descent into it).
  // Validated against the 2015-01 / 2018-12 / 2022-11 cycle lows (dd −77% to −85%); today's
  // −52% sits above the threshold and correctly remains correction.
  if (dd != null && dd < -0.60 && (mayer == null || mayer < 1.0)) return 'accumulation';
