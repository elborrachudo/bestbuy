...
export async function fetchTokenInputs(token, apiKey) {
  const out = {
    price: null, ma50: null, ma200: null, high365: null, rsi14: null,
  ...
  // CoinGecko price series → MAs / 1-year high / RSI / current price.
  // 365 days covers ma200 and the trailing-year high in a single daily series.
  try {
    const prices = await getCoinGeckoPrices(token.coingecko_id, 365, apiKey);
    if (prices.length) {
      out.price = prices[prices.length - 1];
      out.ma50 = sma(prices, 50);
      out.ma200 = sma(prices, 200);
      out.rsi14 = rsi(prices, 14);
      out.high365 = highLow(prices, 365).high;
    }
