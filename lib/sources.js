...
export async function fetchTokenInputs(token, apiKey) {
  const out = {
    ...
    // Confluence indicators (for the signals layer; not part of the score/pillars).
    stochrsi14: null, macdLine: null, macdSignal: null, macdHistogram: null,
    structuralDecline: false,   // §2.2 survivorship: price under a falling long MA for months
  ...
  const { sma, highLow, rsi, confluenceAt } = await import('./scoring.js');
  ...
  try {
    ...
    if (series.length) {
      ...
      // StochRSI + MACD at the latest point, from the same daily price series.
      const conf = confluenceAt(prices);
      out.stochrsi14 = conf.stochrsi_14;
      out.macdLine = conf.macd_line;
      out.macdSignal = conf.macd_signal;
      out.macdHistogram = conf.macd_histogram;
      const { structuralDeclineLatest } = await import('./survivorship.js');
      out.structuralDecline = structuralDeclineLatest(prices);
    }
