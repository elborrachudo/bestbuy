...
export default async function handler(req, res) {
  ...
  const topN = Math.max(10, Math.min(250, Number((req.query && req.query.topN) || 100)));
  const pauseMs = Math.max(0, Number((req.query && req.query.pause) || 1400));

  try {
    const { rows, meta } = await buildDominanceSeries(cgKey, { topN, pauseMs });
